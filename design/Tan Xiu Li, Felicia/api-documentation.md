...
| `POST /api/facial-recognition/access-event` | Record V-Patrol safe access without Attendance. FM/edge. | `{"userId":7,"cameraLocation":"Biometric Gantry"}` | 200 `{"status":"SUCCESS","logged":true,"worker":"A","role":"Staff"}`; deduplicated SecurityLog only. | 400 userId, 401/403, 404 unknown/not enrolled, 429, 500. No AI call in this endpoint. |
...
| `POST /api/attendance/scan` | Authoritative Gate Scanner attendance write. FM JWT or `x-service-key`. | `{"userId":7,"cameraLocation":"Main Gate"}` | 200 `{"status":"SUCCESS","action":"CLOCK_IN_SUCCESSFUL","worker":"A","role":"Staff","timestamp":"...","openTurnstile":true}`; creates IN/OUT or updates today's OUT and safe SecurityLog. | 400 userId, 401/403, 404 user, 429, 500. `openTurnstile` is a software signal, not physical actuation. |
