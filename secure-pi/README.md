...
Off-device suites — `cv2`/`picamera2` are stubbed or unused, so they run on any
...
event log are stored locally** under `runtime/` (git-ignored) and are **not**
uploaded anywhere by this software. Snapshots are auto-pruned to `--max-snapshots`.
If you deploy this, you are responsible for signage/consent and for securing or
purging `runtime/` per your local laws and policies. Do not commit captured
images or logs to version control.
