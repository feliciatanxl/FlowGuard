...
class BagTracker:
    ...
    def _register(self, det: Detection, now: float) -> TrackedBag:
        bag = TrackedBag(self._next_id, det.centroid, det.box, now, now, score=det.score)
...
def _fire_alert(frame, bag: TrackedBag, config: Config, now: float,
    ...
    log_event(config, event_type="unattended_object", label="bag",
    ...
    try:
        while True:
            ...
            for pest in pest_alerts_due:
                _fire_pest_alert(frame, pest, config, now, renderer)
