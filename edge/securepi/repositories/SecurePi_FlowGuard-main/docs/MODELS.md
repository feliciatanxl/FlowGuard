...
1. **Expected output tensor format** — confirm on the Pi what the compiled
   ...
   `imx500.network_intrinsics.postprocess`. A one-shot, bounded debug option is built
   in: run the IMX500 runner with `--debug-tensors` (or `Config(debug_tensors=True)`) to
   log the output-tensor **count, each tensor's shape/dtype and a small value sample
   ONCE** on the first inferred frame — without guessing the tensor order or flooding the
   log. This is the safe way to check whether the compiled network emits 3 SSD tensors,
   a NanoDet tensor, or an undecoded raw YOLOv8 tensor (possibly 4 tensors).
